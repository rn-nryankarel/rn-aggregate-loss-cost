...
data {
    ...
    vector<lower=0>[N] R;                   // reported loss
    vector<lower=0>[N] D;                   // reported dcc
...
parameters {
    ...
    // dcc ratio - expressed as a scale adjustment against  X_beta
    real X_log_scale_dcc_ratio_mu;
    real<lower=0> X_log_scale_dcc_ratio_sigma_rnlob;
    real<lower=0> X_log_scale_dcc_ratio_sigma_svrty;
    ...
    vector[K_svrty] X_log_alpha_svrty_z;
    vector[K_cause] X_log_alpha_cause_z;
    vector[K_class] X_log_beta_class_z;
    vector[K_state] X_log_beta_state_z;
    ...
    vector[K_svrty] g_log_alpha_svrty_z;
    vector[K_rnlob] X_log_scale_trend_rnlob_z;
    vector[K_rnlob] X_log_scale_dcc_ratio_rnlob_z;
    vector[K_svrty] X_log_scale_dcc_ratio_svrty_z;
}
transformed parameters {
    ...
    vector[N] X_log_alpha = cross_classify(
        ...
        X_log_alpha_sigma_svrty, X_log_alpha_svrty_z, g_svrty,
    ...
    vector[N] X_log_beta = cross_classify(
        ...
        X_log_beta_sigma_state, X_log_beta_state_z, g_state,
    ...
    vector[N] g_log_alpha = cross_classify(
        ...
        g_log_alpha_sigma_svrty,  g_log_alpha_svrty_z, g_svrty
    );
    ...
    vector[N] X_log_scale_dcc_ratio = cross_classify(
        X_log_scale_dcc_ratio_mu,
        X_log_scale_dcc_ratio_sigma_rnlob,  X_log_scale_dcc_ratio_rnlob_z, g_rnlob,
        X_log_scale_dcc_ratio_sigma_svrty,  X_log_scale_dcc_ratio_svrty_z, g_svrty
    );

    vector<lower=0>[N] S_lambda = exp(S_log_lambda);
    // differentiating shape of the development curve with this D_delta param
    vector<lower=0>[N] g_R_alpha = exp(g_log_alpha);
    vector<lower=0>[N] g_D_alpha = exp(g_log_alpha + g_log_alpha_D_delta);

    // reasoning on adding the trend variable instead of subtracting:
    ...
    // for the DCC adjustment, though, an increase in the DCC:Loss ratio
    // should lower the inverse scale (beta) thus increasing the mean,
    // hence the subtraction of the log_scale_dcc_ratio parameter for the
    // DCC inverse scale.
    vector<lower=0>[N] X_R_beta = exp(X_log_beta + X_log_scale_trend .* T);
    vector<lower=0>[N] X_D_beta = exp(X_log_beta + X_log_scale_trend .* T - X_log_scale_dcc_ratio);
    vector<lower=0>[N] X_alpha = exp(X_log_alpha);
    vector<lower=0>[N] X_R_alpha_reported;
    vector<lower=0>[N] X_R_alpha_unreported;
    vector<lower=0>[N] X_D_alpha_reported;
    vector<lower=0>[N] X_D_alpha_unreported;
    ...
    vector[N] log_epsilon_R = zeros_vector(N);
    vector[N] log_epsilon_D = zeros_vector(N);
    for (n in 1:N) {
        if (settled[n]) {
            S_lambda_settled[claim_idx_to_settled_idx[n]] = S_lambda[n];
        } else {
            int idx_unsettled = claim_idx_to_unsettled_idx[n];
            log_epsilon_R[n] = truncated_weibull_lcdf(
                S_unknown_lb[idx_unsettled] |
                g_R_alpha[n],
                S_unknown[idx_unsettled]
            );
            log_epsilon_D[n] = truncated_weibull_lcdf(
                S_unknown_lb[idx_unsettled] |
                g_D_alpha[n],
                S_unknown[idx_unsettled]
            );
    ...
    X_R_alpha_reported = exp(X_log_alpha + log_epsilon_R);
    X_D_alpha_reported = exp(X_log_alpha + log_epsilon_D);
    X_R_alpha_unreported = X_alpha - X_R_alpha_reported;
    X_D_alpha_unreported = X_alpha - X_D_alpha_reported;
}
model {
    // level 2 priors
    ...
    X_log_beta_sigma_state ~ exponential(10);
    X_log_alpha_sigma_rnlob ~ exponential(1);
    X_log_beta_sigma_rnlob ~ exponential(1);
    X_log_alpha_sigma_svrty ~ exponential(0.5);
    ...
    g_log_alpha_sigma_svrty ~ exponential(10);
    g_log_alpha_D_delta ~ normal(-0.5, 0.5);

    X_log_scale_trend_mu ~ normal(0.06, 0.03);
    X_log_scale_trend_sigma_rnlob ~ exponential(100);

    X_log_scale_dcc_ratio_mu ~ normal(log(0.2), 0.3);
    X_log_scale_dcc_ratio_sigma_rnlob ~ exponential(10);
    X_log_scale_dcc_ratio_sigma_svrty ~ exponential(10);

    // level 1 priors
    X_log_alpha_class_z ~ std_normal();
    X_log_alpha_state_z ~ std_normal();
    X_log_alpha_rnlob_z ~ std_normal();
    X_log_alpha_svrty_z ~ std_normal();
    X_log_alpha_cause_z ~ std_normal();
    X_log_beta_class_z ~ std_normal();
    X_log_beta_state_z ~ std_normal();
    ...
    g_log_alpha_svrty_z ~ std_normal();
    X_log_scale_trend_rnlob_z ~ std_normal();
    X_log_scale_dcc_ratio_rnlob_z ~ std_normal();
    X_log_scale_dcc_ratio_svrty_z ~ std_normal();
    ...
    R ~ gamma(X_R_alpha_reported, X_R_beta);
    D ~ gamma(X_D_alpha_reported, X_D_beta);
}
generated quantities {
    array[N] real<lower=0> R_post_pred;
    array[N] real<lower=0> IBNER_R_post_pred = zeros_array(N);
    vector[N] R_log_lik;
    array[N] real<lower=0> D_post_pred;
    array[N] real<lower=0> IBNER_D_post_pred = zeros_array(N);
    vector[N] D_log_lik;
    for (n in 1:N) {
        R_post_pred[n] = gamma_rng(X_R_alpha_reported[n], X_R_beta[n]);
        R_log_lik[n] = gamma_lpdf(R[n] | X_R_alpha_reported[n], X_R_beta[n]);
        D_post_pred[n] = gamma_rng(X_D_alpha_reported[n], X_D_beta[n]);
        D_log_lik[n] = gamma_lpdf(D[n] | X_D_alpha_reported[n], X_D_beta[n]);
        if (X_R_alpha_unreported[n] > 0) {
            IBNER_R_post_pred[n] = gamma_rng(X_R_alpha_unreported[n], X_R_beta[n]);
        }
        if (X_D_alpha_reported[n] > 0) {
            IBNER_D_post_pred[n] = gamma_rng(X_D_alpha_unreported[n], X_D_beta[n]);
        }
    }
}
