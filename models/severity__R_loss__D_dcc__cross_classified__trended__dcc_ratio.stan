...
transformed parameters {
    ...
    // DCC inverse scale.
    vector<lower=0>[N] X_beta_R = exp(X_log_beta + X_log_scale_trend .* T);
    vector<lower=0>[N] X_beta_D = exp(X_log_beta + X_log_scale_trend .* T - X_log_scale_dcc_ratio);
...
model {
    ...
    R ~ gamma(X_alpha_reported, X_beta_R);
    D ~ gamma(X_alpha_reported, X_beta_D);
}
generated quantities {
    ...
    vector[N] D_log_lik;
    for (n in 1:N) {
        R_post_pred[n] = gamma_rng(X_alpha_reported[n], X_beta_R[n]);
        R_log_lik[n] = gamma_lpdf(R[n] | X_alpha_reported[n], X_beta_R[n]);
        D_post_pred[n] = gamma_rng(X_alpha_reported[n], X_beta_D[n]);
        D_log_lik[n] = gamma_lpdf(D[n] | X_alpha_reported[n], X_beta_D[n]);
        if (X_alpha_unreported[n] > 0) {
            IBNER_R_post_pred[n] = gamma_rng(X_alpha_unreported[n], X_beta_R[n]);
            IBNER_D_post_pred[n] = gamma_rng(X_alpha_unreported[n], X_beta_D[n]);
        }
    }
}
