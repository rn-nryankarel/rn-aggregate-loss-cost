...
data {
    ...
    vector<lower=0>[N] t;                         // policy term length
...
transformed data {
    ...
    vector<lower=1e-3>[C_rows] R_trunc;        // truncation point for each claim
    for (c in 1:C_rows) {
        C_R[idx_policy[c]] += 1;
...
parameters {
    ...
    real<lower=0> R_shape;
    real<lower=0> R_scale;
    real<lower=1e-5> C_R_phi; // NB2 dispersion parameter
}
transformed parameters {
    ...
    vector[N] log_C_R_mean = log_C_ultimate + log_theta_R;
}
model {
    // priors
