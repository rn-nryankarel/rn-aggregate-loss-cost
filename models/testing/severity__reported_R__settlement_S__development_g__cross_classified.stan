...
parameters {
    ...
    // ultimate severity parameters - cc'd across all groups
    real X_log_alpha_mu;
    real X_log_beta_mu;
...
transformed parameters {
    ...
    vector<lower=0>[N] X_beta = exp(X_log_beta);
...
model {
    ...
    // level 1 priors
    // TODO: use multivariate prior with correlation between severity code's report lag and severity mean
    ...
    R ~ gamma(X_alpha_reported, X_beta);
}
generated quantities {
    ...
    vector[N] R_log_lik;
    for (n in 1:N) {
        R_post_pred[n] = gamma_rng(X_alpha_reported[n], X_beta[n]);
        R_log_lik[n] = gamma_lpdf(R[n] | X_alpha_reported[n], X_beta[n]);
        if (X_alpha_unreported[n] > 0) {
            IBNER_post_pred[n] = gamma_rng(X_alpha_unreported[n], X_beta[n]);
        }
    }
}
