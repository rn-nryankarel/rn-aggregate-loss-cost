...
// predictors. we'll concatenate the two groups to consider all pairs of
// targets, with no pooling as another extreme.
...
parameters {
    // one set for the "concatenated" joint code
    vector[K_joint] alpha_joint;
    matrix[num_predictors, K_joint] beta_joint;
}
model {
    alpha_joint ~ std_normal();
    to_vector(beta_joint) ~ std_normal();

    g_joint_1d ~ categorical_logit_glm(X, alpha_joint, beta_joint);
}
generated quantities {
    array[N, 2] int<lower=1, upper=max(K_svrty, K_cause)> k_joint_post_pred;
    array[N] int<lower=1, upper=K_joint> k_joint_1d_post_pred;

    vector[N] k_joint_log_lik;

    matrix[N, K_joint] logit_joint = rep_matrix(alpha_joint', N) + X * beta_joint;

    for (n in 1:N) {
        int sampled_joint_code = categorical_logit_rng(logit_joint[n]');
        k_joint_1d_post_pred[n] = sampled_joint_code;
        k_joint_post_pred[n, ] = joint_code_to_original_groups[sampled_joint_code];
        k_joint_log_lik[n] = categorical_logit_lpmf(g_joint_1d[n] | logit_joint[n]');
    }
}
